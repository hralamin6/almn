<div class="h-full p-4" x-data="user()">
        ...
        <div class="py-4 flex flex-col justify-start md:w-1/2 m-auto">
            @foreach($items as $i => $item)
                <div @if(!$is_single_page) x-cloak x-show="step=={{$i+1}}"
                    ...
                    <legend class="text-lg font-medium my-1"><span>({{$i+1}})</span> @lang("What is the arabic form of ") <span
                            class="text-primary">{{$item->meaning}}?</span></legend>
                    @if(!$is_mcq)
    ...
    @else
        ...
        if (strlen(implode($diff2))==0 && strlen($this->ans[$i])<13){
            ...
            @endforeach
            <center><a href="{{route('exam')}}?practise={{$practise}}" class="btn btn-outline btn-primary btn-xs btn-block w-48">Try again</a></center>
        </div>
    @endif
        @script
        <script>
